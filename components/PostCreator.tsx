...
import { Type, Image, Film, Sparkles, UploadCloud, X, BookImage } from 'lucide-react';
...
export const PostCreator: React.FC<PostCreatorProps> = ({ onGenerate, isLoading, isApiKeySelectedForVideo, creditCosts }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (prompt.trim() || uploadedImages.length > 0) {
            onGenerate(prompt, postType, uploadedImages);
        }
    };
    
    const isGenerateDisabled = isLoading || 
        (!prompt.trim() && uploadedImages.length === 0) || 
        ((postType === PostType.VIDEO || postType === PostType.STORY) && !isApiKeySelectedForVideo) ||
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {uploadedImages.length > 0 ? (
                 <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                    {uploadedImages.map((image, index) => (
                        <div key={index} className="relative group aspect-square">
                            <img src={image} alt={`Upload preview ${index + 1}`} className="w-full h-full object-cover rounded-lg" />
                            <button 
                                type="button"
                ...
                >
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 text-brand-text-secondary">
                        <UploadCloud size={40} className={`mb-3 transition-colors ${isDragging ? 'text-brand-primary' : ''}`} />
                        <p className="mb-2 text-sm"><span className="font-semibold text-brand-text">Click to upload</span> or drag and drop</p>
                        <p className="text-xs">PNG, JPG, or WEBP</p>
                    </div>
                    <input ref={fileInputRef} type="file" multiple className="hidden" accept="image/png, image/jpeg, image/webp" onChange={handleFileChange} />
                </div>
            )}
            ...
            <div>
                <label htmlFor="prompt" className="block text-sm font-medium text-brand-text-secondary mb-2">
                    {uploadedImages.length > 0 ? "Describe the story you want to create..." : "What's on your mind?"}
                </label>
                <textarea
                    id="prompt"
                    value={prompt}
            ...
            <div>
                <span className="block text-sm font-medium text-brand-text-secondary mb-2">Choose post type</span>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {postTypeOptions.map((option) => {
                        const isSelected = postType === option.id;
                                                const isDisabled = isLoading || 
                            ((option.id === PostType.VIDEO || option.id === PostType.STORY) && !isApiKeySelectedForVideo);
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className="text-xs text-brand-text-secondary">{creditCosts[option.id]} credits</span>
                            </button>
                        );
                    })}
                </div>
                {(postType === PostType.VIDEO || postType === PostType.STORY) && !isApiKeySelectedForVideo && (
                     <p className="text-xs text-yellow-400/80 mt-2">A Google AI Studio API key is required for this feature. The key selection dialog will appear at the bottom of the page.</p>
                )}
                {postType === PostType.STORY && uploadedImages.length === 0 && (
