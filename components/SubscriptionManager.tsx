...
const plans: Record<SubscriptionPlan, PlanDetails> = {
  ...
  [SubscriptionPlan.PRO]: {
    ...
    historyLimit: 120,
    features: [
      `${Math.floor(100 / CREDIT_COSTS[PostType.VIDEO])} Video/Story Generations`,
      `${Math.floor(100 / CREDIT_COSTS[PostType.IMAGE])} Image Generations`,
      `${Math.floor(100 / CREDIT_COSTS[PostType.TEXT])} Text Generations`,
      "Priority Support",
      "History Storage: 120 posts",
    ],
  },
  [SubscriptionPlan.BUSINESS]: {
    ...
    historyLimit: 250,
    features: [
      `${Math.floor(250 / CREDIT_COSTS[PostType.VIDEO])} Video/Story Generations`,
      `${Math.floor(250 / CREDIT_COSTS[PostType.IMAGE])} Image Generations`,
      `${Math.floor(250 / CREDIT_COSTS[PostType.TEXT])} Text Generations`,
      "Dedicated Support",
      "History Storage: 250 posts",
    ],
  },
};
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {Object.values(plans).map((plan) => (
          <div
            key={plan.name}
            className={`p-4 rounded-lg border-2 flex flex-col h-full min-h-[370px] ${currentPlan === plan.name ? "border-brand-primary" : "border-brand-border"}`}
          >
            ...
            <ul className="mt-4 space-y-2 text-left text-sm flex-1">
            ...
            <button
              ...
              className="w-full mt-4 py-2 px-4 bg-brand-primary text-white font-semibold rounded-lg disabled:bg-gray-500 disabled:cursor-not-allowed mt-auto"
            >
